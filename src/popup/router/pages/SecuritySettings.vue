<template>
    <div class="popup">
        ...
        <div v-if="type == 4">
            ...
            <ae-phraser>
                <ae-badge class="seedBadge" :class="{'selected':seed.selected}" v-for="(seed,index) in seeds" v-bind:key="seed.id" @click.native="selectSeed(seed.name,index,seed.id)">{{seed.name}}</ae-badge>
            </ae-phraser>
    ...
    methods: {
        seedPhraseRecovery() {
            ...
            browser.storage.local.get('mnemonic').then(async (seed) => {
                ...
                var seedPhraseToArray = seed.mnemonic.split(" ");

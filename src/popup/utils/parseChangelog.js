export default async () => {
  ...
  const changelogUrl = (await (await fetch(
    'http://localhost:5000/CHANGELOG.md',
    { headers },
  )).text());
  // const changelogFull = atob((await (await fetch(changelogUrl, { headers })).json()).content);
  const versionsChangelog = await [...changelogUrl.matchAll(/\[(\d+\.\d+\.\d+)\].*?\n(.*?)(?=\[\d+\.\d+\.\d+\])/sg)].slice(0, 3)
        ...
        .reduce(async (td, [, topic, details]) => ({
          ...
          [topic]: await Promise.all([...details.matchAll(/\[[\w\d]+\](\(.*?)\)/sg)]
            .map(async ([, commit]) => {
              ...
              const prs = (await (await fetch(apiUrl, { headers: { ...headers, Accept: 'application/vnd.github.groot-preview+json' } }))
                .json()).sort((a, b) => new Date(a.created_at) - new Date(b.created_at));
              let issuesImgs = [];
              if (!prs[0]) return { undefined: [] };
              ...
              if (topic !== 'Features') {
                ...
                const issues = await Promise.all(closesIssues.map(async (i) => (await fetch(`https://api.github.com/repos/aeternity/superhero-wallet/issues/${i}`, { headers })).json()));

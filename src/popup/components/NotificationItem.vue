<template>
  ...
  >
    ...
    <div class="content">
      ...
      <BtnMain
        v-if="isSeedBackup"
        :text="redirectInfo.label"
        class="redirect-button"
        variant="special"
      />
      <div
        v-else-if="redirectInfo.label"
...
export default defineComponent({
  ...
  setup(props: IProps, { root }) {
    ...
    const chainName = computed(() => props.notification.chainName);
    const address = computed(() => props.notification.sender);
    ...
    const redirectInfo = computed(() => ({
      label: !isWallet ? root.$t('pages.notifications.viewOnSuperhero') : props.notification.buttonLabel,
    }));
    const title = computed(() => isWallet.value
      ? props.notification.title || ''
      : chainName.value || address.value || 'Fellow Superhero');
    const isUnread = computed(() => props.notification.status !== NOTIFICATION_STATUS_READ);

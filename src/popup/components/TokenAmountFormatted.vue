  ...
  setup(props) {
    const assetAmountFormatted = computed(() => {
      ...
      if (amount.gt(1000000000000)) {
        return {
          number: `${amount.dividedBy(1000000000000).toFixed(2)}T`,
        };
      }

      if (amount.gt(1000000000)) {
        return {
          number: `${amount.dividedBy(1000000000).toFixed(2)}B`,
        };
      }

      if (amount.gt(1000000)) {
        return {
          number: `${amount.dividedBy(1000000).toFixed(2)}M`,
        };
      }

      if (amount.gt(100000)) {
        return {
          number: `${amount.dividedBy(1000).toFixed(2)}K`,
        };
      }

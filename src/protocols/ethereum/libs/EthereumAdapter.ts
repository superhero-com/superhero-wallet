...
export class EthereumAdapter extends BaseProtocolAdapter {
  ...
  override async transferPreparedTransaction(transactionData: any): Promise<ITransferResponse> {
    ...
    const account = getAccountByAddress(toChecksumAddress(transactionData.from));
    ...
    const [nonce] = await Promise.all([
      this.getTransactionCount(transactionData.from),
      updateFeeList(),
    ]);

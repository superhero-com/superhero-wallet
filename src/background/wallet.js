...
const addAeppConnection = (port) => {
  ...
  store.getters['sdk/sdk'].addRpcClient(connection);
  store.getters['sdk/sdk'].shareWalletInfo(port.postMessage.bind(port));
  const shareWalletInfo = setInterval(() => store.getters['sdk/sdk'].shareWalletInfo(port.postMessage.bind(port)), 3000);
...
export async function init() {
  ...
  await store.dispatch('sdk/initialize');
  // eslint-disable-next-line no-underscore-dangle
  await store._watcherVM.$watchUntilTruly(() => store.getters['sdk/sdk']);
...
export function disconnect() {
  Object.values(store.getters['sdk/sdk'].rpcClients).forEach((aepp) => {
    ...
    store.getters['sdk/sdk'].removeRpcClient(aepp.id);
  });
}

...
export default (store) => {
  ...
  store.registerModule('sdk13Plugin', {
    namespaced: true,
    state: {
      ready: false,
    },
    ...
    actions: {
      async initialize({ commit }) {
        ...
        // TODO: use this method for `onMessageSign` replacement.
        // eslint-disable-next-line no-unused-vars
        const signCb = async (payload, options, origin = null) => {
          try {
            const host = origin ? new URL(origin).host : null;
            const permission = (!host && IS_CORDOVA) || await store.dispatch('permissions/checkPermissions', {
              host,
              method: 'sign',
              params: payload,
            });
        ...
        const signCbBackground = async (method, payload, options) => {
          ...
          const isTxSigning = method === 'transaction.sign';
          if (
        ...
        class SuperheroSdk extends AeSdkWallet {
          _resolveAccount = () => ({
            ...
            signTransaction: (txBase64, opt) => IS_EXTENSION_BACKGROUND
              ? signCbBackground('transaction.sign', txBase64, opt)
  ...
  store.watch(
    ...
    async (network, oldNetwork) => {
      ...
      await watchUntilTruthy(() => store.getters['sdk13Plugin/sdk']);

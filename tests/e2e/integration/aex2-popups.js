...
describe("Tests cases for AEX-2 popups", () => {
  ...
  txTypes.forEach((txType) => {
    it(`Sign Popup display correct ${txType} data`, () => {
      const props = popupProps["sign"]
      const tx = txParams[txType]
      const amount = (tx.amount / 10 ** 18)
      const fee = (tx.fee / 10 ** 18)
      const receiver = txType === "spendTx" ? tx.recipientId : (txType === "contractCallTx" ? tx.contractId : 'New Contract')
      cy
      .openAex2Popup("sign", txType)
      .get('[data-cy=tx-type]')
      .should('be.visible')
      ...
      if(txType === "spendTx" || txType === "contractCallTx") {
        cy
        .get('[data-cy=address-receiver]')
        .should('be.visible')
        .invoke('attr', 'title')
        .should('contain',receiver)
      } else {
        cy
        .get('[data-cy=receiver]')
        .should('be.visible')
        .should('contain',receiver)
      }

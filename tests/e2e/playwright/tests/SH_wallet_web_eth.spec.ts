...
*/
  ...
  test('SH Wallet_rst_eth_walletconnect', async ({ page, context }) => {
    ...
    // Check if test account is connected and disconnect
    await page.waitForTimeout(1000);
    if (await page2.locator('//button[@data-testid="web3-status-connected"]').isVisible() === true) {
      await page2.locator('//button[@data-testid="web3-status-connected"]').click();
      await page2.locator('//button[@data-testid="wallet-disconnect"]').click();

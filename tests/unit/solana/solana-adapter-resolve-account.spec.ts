...
const web3 = (await import('@solana/web3.js')) as any;

(vi.spyOn(web3.Keypair as any, 'fromSecretKey') as any).mockImplementation(() => ({
...
(vi.spyOn(web3.Keypair as any, 'fromSeed') as any).mockImplementation(() => ({
  secretKey: new Uint8Array(64).fill(2),
  publicKey: {
...
describe('SolanaAdapter - resolveAccountRaw private-key parsing', () => {
  ...
  it('accepts 64-byte secret key Uint8Array', async () => {
  ...
  it('accepts 32-byte seed Uint8Array', async () => {
  ...
  it('accepts Buffer-like object', async () => {
